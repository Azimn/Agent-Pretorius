...
The default routine set is intentionally bounded. `Pretorius Pulse` wakes every hour and completes at most one meaningful unit of work. `Pretorius Continuity Probe` runs every six hours and records a longitudinal response from the no-answer-key continuity battery. `Pretorius Night Reflection` runs daily and may update the self-model only when repeated evidence warrants it. `Pretorius Lab Brief` runs every morning and summarizes actual changes without manufacturing progress.
